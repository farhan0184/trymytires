...
export default function ProductFinishPage() {
  const [data, getData, { loading }] = useFetch(fetchProductFinish);
  ...
  const onSubmit = async (values) => {
    ...
    await handleApiRequest({
      apiFunc: isEdit ? updateProductFinish : createProductFinish,
      payload,
  ...
  const columns = [
    {
      accessorKey: "name",
      header: () => <div className="subtitleText font-bold px-4 py-3 text-gray-800">Finish</div>,
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Switch checked={row.original.status} onCheckedChange={() => handleStatusToggle(row.original)} />
      ),
    },
    {
      id: "actions",
  ...
  return (
    <>
      <DataTable
        ...
        description="Manage product finish options"
        ...
        emptyMessage="No finish entries found"
        noResultsMessage="No matching finish entries"
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{isEdit ? "Edit Finish" : "Add New Finish"}</DialogTitle>
          </DialogHeader>
          <Form {...form}>
