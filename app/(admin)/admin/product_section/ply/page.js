...
export default function ProductPlyPage() {
    const [data, getData, { loading }] = useFetch(fetchProductPly);
    ...
    const onSubmit = async (values) => {
        ...
        await handleApiRequest({
            apiFunc: isEdit ? updateProductPly : createProductPly,
            payload,
    ...
    const columns = [
        {
            accessorKey: "name",
            header: () => (
                <div className="subtitleText font-bold px-4 py-3 text-gray-800">PLY</div>
            ),
            cell: ({ row }) => (
        ...
        },
        {
            accessorKey: "status",
            header: () => (
                <div className="subtitleText font-bold px-4 py-3 text-gray-800">Status</div>
            ),
            cell: ({ row }) => (
                <Switch
            ...
            cell: ({ row }) => (
                <div className="flex items-center justify-end space-x-2">
                    ...
                    <AlertDialog>
                        ...
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                <AlertDialogTitle>Delete PLY</AlertDialogTitle>
                                <AlertDialogDescription>
    ...
    return (
        <>
            <DataTable
                ...
                loading={loading}
                title="Product PLY"
                description="Manage your product PLY values"
                icon={LayoutDashboard}
                searchConfig={{
                    placeholder: "Search by PLY name...",
                    fields: ["name"],
                    serverSide: true,
                }}
                filterConfig={[
                ...
                emptyMessage="No PLY entries found"
                noResultsMessage="No matching PLY entries"
            ...
            <Dialog open={modalOpen} onOpenChange={setModalOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>{isEdit ? "Edit PLY" : "Add New PLY"}</DialogTitle>
                    </DialogHeader>
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-2">
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel>PLY Name</FormLabel>
                                        <FormControl>
                                            <Input placeholder="Enter PLY name" {...field} />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}
